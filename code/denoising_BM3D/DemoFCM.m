function [L,C,U,LUT,H] = DemoFCM (frame,num_cluster)
...
% % Number of segments
% num_cluster=6;
% im=im2uint8(last_frame);
im=im2uint8(frame);
% im=imread('cameraman.tif'); % sample image
[L,C,U,LUT,H]=FastFCMeans(im,num_cluster); % perform segmentation
...
% str=sprintf('FUZZY C-MEANS (C=%i)',num_cluster);
